...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const redirectPath =
      user.role === "admin" ? "/admin.html" : "/citizen.html";
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await connection.execute("DELETE FROM feedback WHERE id = ?", [
        feedbackId,
      ]);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existing.length > 0)
        return res.status(400).json({
          success: false,
          message: "You already upvoted this feedback",
        });
...
app.post("/logout", authenticateToken, (req, res) => {
  res.json({
    success: true,
    message: "Logged out successfully (discard token client-side)",
  });
});
